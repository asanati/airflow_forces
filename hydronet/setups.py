...
# smiley = (torch.mean(torch.tensor(np.asarray(Image.open('imgs/smiley_old.png'))).float(),dim=2)<100).float()
smiley = (torch.mean(torch.tensor(np.asarray(Image.open('imgs/drone.png').convert('RGBA'))).float(),dim=2)<150).float() # WORKS!
# smiley = (torch.mean(torch.tensor(np.asarray(Image.open('imgs/fish_down.png'))).float(),dim=2)<100).float()
wing = (torch.mean(torch.tensor(np.asarray(Image.open('imgs/wing_profile.png'))).float(),dim=2)<100).float()
background = (torch.mean(torch.tensor(np.asarray(Image.open('imgs/background.png').convert('RGBA'))).float(),dim=2)<100).float()
...
# # wing = (torch.mean(,dim=2)<100).float()
# wing = torch.tensor(np.asarray(Image.open('imgs/wing_profile.png'))).float()
# print(wing.shape)
# print(wing)
# wing = torch.mean(wing,dim=2)
# print(wing.shape)
# print(wing)
# wing = (wing<100).float()
# print(wing.shape)
...
def preprocess():
    propeller = torch.zeros(40, 40)
    for i in range(smiley.shape[1]):
        if i >= 4 and i <= 13:
            propeller[13][i] = 1
            smiley[13][i] = 1
        elif i >= 27 and i <= 35:
...
backgrounds = {"empty":background,"empty100":background1,"cave1":background2,"cave2":background3}
...
class Dataset:
    def __init__(self,w,h,batch_size=100,dataset_size=1000,average_sequence_length=5000,interactive=False,max_speed=3,brown_damping=0.9995,brown_velocity=0.005,init_velocity=0,init_rho=None,n_cond=False,dt=1,types=["magnus","box","pipe"],images=["cyber","fish","smiley","wing"],background_images=["empty"]):
        ...
        self.propeller = preprocess()
    
    # Assume x is the constant coordinate
    def determine_position(self, y, z):
        x_pos = max(0, min(y/3.0*self.w, self.w))
        y_pos = self.h - max(0, min(z/3.0*self.h, self.h))
        return x_pos, y_pos
    ...
    def reset_env(self,index):
        ...
        if type == "magnus": # magnus effekt (1)
            ...
            # self.v_cond[index,0,(object_y-object_r):(object_y+object_r+1),(object_x-object_r):(object_x+object_r+1)] += 0
            # self.v_cond[index,1,(object_y-object_r):(object_y+object_r+1),(object_x-object_r):(object_x+object_r+1)] += 0
    ...
    def update_envs(self,indices):
        ...
        for index in indices:
            ...
            if self.env_info[index]["type"] == "image":
                ...
                self.cond_mask[0,0,280,140:160]=1

                # print("Begin")
                # for i in range(200):
                #     for j in range(200):
                #         print(int(self.cond_mask[0,0,i,j].item()), end="")
                #     print("")
                # print("End")
                ...
                # TODO: remove extra propeller velocity
                self.v_cond[index,0,int(object_y-object_h//2):int(object_y-object_h//2+object_h),int(object_x-object_w//2):int(object_x-object_w//2+object_w)] += 2.5*self.propeller
                # self.v_cond[index,1,int(object_y-object_h//2):int(object_y-object_h//2+object_h),int(object_x-object_w//2):int(object_x-object_w//2+object_w)] += 2.5*self.propeller

                # Uncomment for horizontal direction
                # self.v_cond[index,1,20:(self.h-20),0:5]=flow_v
                # self.v_cond[index,1,20:(self.h-20),(self.w-5):self.w]=flow_v

                # self.v_cond[index,1,0:(self.h),0:5]=flow_v
                # self.v_cond[index,1,0:(self.h),(self.w-5):self.w]=flow_v

                # print(self.v_cond[index,1,:].shape)
                # print_obj(self.v_cond)

                # For determining quad position
                x_pos, y_pos = self.determine_position(1.5, 0.555)
                # print(x_pos, y_pos)
                self.mousex = x_pos
                self.mousey = y_pos
                ...
                # self.v_cond[index,0,0:5,20:(self.w-20)]=flow_v
                # self.v_cond[index,0,(self.w-5):self.w,20:(self.w-20)]=flow_v

                # print("y begin:")
                # print(int(object_y-object_h//2))
                # print("y end:")
                # print(int(object_y-object_h//2+object_h))

                # print("x begin:")
                # print(int(object_x-object_w//2))
                # print("x end:")
                # print(int(object_x-object_w//2+object_w))

                # self.v_cond[index,0,int(object_y-object_h//2):int(object_y-object_h//2+object_h),int(object_x-object_w//2):int(object_x-object_w//2+object_w)] = flow_v
                # self.v_cond[index,1,int(object_y-object_h//2):int(object_y-object_h//2+object_h),int(object_x-object_w//2):int(object_x-object_w//2+object_w)] = object_vx*image_mask
