...
# def measure_force(t_index):
#     value, unit, _ = fg.read_gauge()
#     force_logger.log("measured_force", (value, unit), t_index)
#     force_logger.log()
...
with torch.no_grad():
    ...
    while num_iter < 10:
        ...
        dataset = Dataset(w,h,1,1,interactive=True,average_sequence_length=n_time_steps,max_speed=params.max_speed,dt=dt,types=["image"],images=["smiley"],background_images=["empty"])
        ...
        mean_forces.append(torch.mean(torch.tensor(fs)).item())
        print(mean_forces)
        np.savetxt("data/foo.csv", torch.tensor(fs).detach().cpu().numpy(), delimiter=",")
        # if quit:
        #     break
